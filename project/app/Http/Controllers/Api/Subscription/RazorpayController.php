...
class RazorpayController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $orderData = [
            ...
            'amount'          => ($order['item_amount'] * $currency->value) * 100,
            'currency'        => 'INR',
            'payment_capture' => 1
        ];
        ...
        $notify_url = route('api.subscription.razorpay.notify')."?order_id=".$razorpayOrder['id']."&currency_id=".$request->currency_id."&user_id=".$request->user_id."&bank_plan_id=".$request->bank_plan_id;

        $input['user_id'] = $user->id;

        $displayAmount = $amount = $orderData['amount'] * $currency->value;
    ...
    public function notify(Request $request)
    {
        ...
        if (empty($input_data['razorpay_payment_id']) === false)
        {
            ...
            {
                $attributes = array(
                    'razorpay_order_id' => $input_data['order_id'],
        ...
        $request["method"] = "Razorpay";

        if ($success === true){
            $addionalData = ['txnid'=>$input_data['razorpay_payment_id'],'subscription_number'=>$input_data['order_id']];
